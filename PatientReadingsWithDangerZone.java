...
//This code will take in different readings for different users and calculate the percentage change and set dangerzone for the patients whose variance is more than 10. 

public class PatientReadingsWithDangerZone {
	...
	public static void main(String[] args){
		...
		ArrayList<PatientDangerPojo> patientArray = new ArrayList<PatientDangerPojo>();
...

			{
				...
				PatientDangerPojo patientNameAndReading = new PatientDangerPojo();
				
				patientNameAndReading.setThePatient(name); 
			...
			PatientReadingsWithDangerZone patientObj = new PatientReadingsWithDangerZone();

			patientObj.thePatientReading(patientArray); 
			patientObj.theDangerPatient(patientArray); 
	}

	//This code is the logic behind the percentage change. 

	public void thePatientReading(ArrayList<PatientDangerPojo> arrayOfPatients){
		...
		*/
		...
		for(int x=0; x<arrayOfPatients.size(); x++ ){
			...
			System.out.println("The name of the patient is: " + po.getThePatient()); 
	...
	//This method will set the dangerzone for the patients depending on the variance and whether or not it is greater than 10. 

	public void theDangerPatient(ArrayList<PatientDangerPojo> dangerPatientArray){

		String dangerColour = "red"; 

		for(int s = 0; s<dangerPatientArray.size(); s++){

		PatientDangerPojo dp = dangerPatientArray.get(s); 

		//is conditions to determine the dangerzone. 

			if(dp.getFinalValue().size() >= 2)

			{

			if(dp.getFinalValue().get(s)>= 10)

				{
					if(dp.getFinalValue().get(s+1) >= 10)

						{
							dp.setDangerZone(dangerColour); 
						}
				}
